...
class System:
    def __init__(self, _database: DataBase.DataBase = None, _debug: bool = True):
        ...
        self.database = (
            DataBase.DataBase(_debug = _debug) if _database is None else _database
        )
        self.cancellation = []
    ...
    def get_lesson(
        ...
        if day_week is None or number_lesson is None:
            return

        if _debug:
            print_debug(
                "System",
                f"Get lesson: day: [main]{day_week}[/main], number lesson: [main]{int(number_lesson)}[/main], type: [main]{type}[/main]",
            )

        if int(number_lesson) > 0 and int(number_lesson) <=  int(
            self.database.get_max_lesson_in_days(_debug = False)
        ):
            ids, lessons = self.database.get_lesson_for_date(
                day_week, int(number_lesson), _debug = False
            )
    ...
    def get_lesson_for_date(
        ...
        if day is None or number_lesson is None:
            return

        if _debug:
            print_debug(
                "System",
                f"Get lesson for date: day: [main]{day}[/main], number lesson: [main]{number_lesson}[/main]",
            )
        date = self.get_day_isoweekly(year = year, mounth = mounth, day = day)
        ids = self.database.get_id_lesson_for_date(
            day = date, number_lesson = number_lesson, _debug = False
        )
        _, lessons = self.database.get_lesson_for_date(
            day = date, number_lesson = number_lesson, _id_lessons = ids, _debug = False
        )
    ...
    def get_lesson_now(
        self, _day = None, _number_lesson = None, _type = None, _debug: bool = True
    ) -> tuple[str, str] | tuple[None, None]:
        """
        ...
        number_lesson = (
            self.get_lesson_number_now_without_type()
            if _number_lesson is None
            else _number_lesson
        )
        ids = self.database.get_id_lesson_for_date(
            day = day, number_lesson = number_lesson, _debug = False
        )
        _, lessons = self.database.get_lesson_for_date(
            day = day, number_lesson = number_lesson, _id_lessons = ids, _debug = False
        )
    ...
    def get_url(
        self, _day = None, _number_lesson = None, _type = None, _id = None, _debug: bool = True
    ) -> str | None:
        """
        ...
        type = self.get_week_type(_debug = False) if _type is None else _type
        id, lesson = self.get_lesson(
            day_week = day, number_lesson = number_lesson, type = type, _debug = False
        )

        if _debug:
            print_debug(
                "System",
                f"Get [main]url[/main], day: [main]{day}[/main], number lesson: [main]{number_lesson}[/main], type: [main]{type}[/main], lessons: [main]{lesson}[/main].",
            )
    ...
    def get_day_isoweekly(
        self, year: int = None, mounth: int = None, day: int = None
    ) -> int:
        """
        ...
        if mounth > 0 and mounth < 13 and day > 0 and day < max_days_in_mounth:
            date = datetime.date(
                ...
                mounth if mounth else datetime.datetime.now().month,
                day if day else datetime.datetime.now().day,
            )
    ...
    def get_week_type(
        ...
        when_mounth, when_day, when_year = (
            (
                mounth
                if mounth
                else self.database.get_data_week_type(_debug = False)["month"]
            ),
            day if day else self.database.get_data_week_type(_debug = False)["day"],
            year if year else self.database.get_data_week_type(_debug = False)["year"],
        )
        when_mounth, when_day, when_year = (
            np.uint(when_mounth),
            np.uint(when_day),
        ...
        type = (
            type
            if type is not None
            else self.database.get_data_week_type(_debug = False)["type"]
        )
        days = np.uint16(0)

        mounth, day, year, max_type = (
            np.uint(mounth),
            np.uint(day),
            np.uint(year),
            np.uint(max_type),
        ...
        if (
            self.get_day_isoweekly(year = when_year, mounth = when_mounth, day = when_day)
             ==  1
        ):
            type +=  1
            if type > max_type:
        ...
        for i in range(mounth - when_mounth + 1):
            for j in range(1, calendar.monthrange(year, i + when_mounth)[1] + 1):
                if i  == 0 and j <=  when_day - 1:
                    ...
                    if (
                        self.get_day_isoweekly(year = year, mounth = when_mounth + i, day = j)
                         ==  1
                    ):
                        days +=  1
    ...
    def get_number_lesson_now(self) -> np.int8:
        ...
        for number_lesson in times:
            if (
                self.get_time_float(
                    _time = times[number_lesson]["start_recess"], _debug = False
                )
                <= time_now
                <= self.get_time_float(
                    _time = times[number_lesson]["start_lessons"], _debug = False
                )
            ):
            ...
            if (
                ...
                <= time_now
                <= self.get_time_float(
                    _time = times[number_lesson]["end_lessons"], _debug = False
                )
            ):
                return np.int8(int(number_lesson))
    ...
    def get_time_float(self, _time = None, _debug: bool = True) -> float:
        ...
        if _time is None:
            ...
            if type(_time) is str and _time !=  "" and ":" in _time:
    ...
    def get_lesson_number_now_without_type(self, _debug: bool = True) -> np.uint8:
        ...
        if _debug:
            print_debug(
                "System",
                f"Get [main]lesson number now without type[/main], number lesson: [main]{number_lesson}[/main].",
            )
    ...
    def get_cancellation(self, _debug: bool = True) -> bool:
        ...
        for i in range(len(self.cancellation)):
            if self.cancellation[i][0] == self.get_lesson_number_now_without_type(
                _debug = False
            ) and self.cancellation[i][1] == self.get_day_isoweekday_now(_debug = False):
                return True
