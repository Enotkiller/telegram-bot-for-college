...
class System:
    ...
    def get_url(self, _day=None, _number_lesson=None, _type=None, _id=None, _debug: bool = True) -> str | None:
        """
        ...
        day = self.get_day_isoweekday_now(_debug=False) if _day is None else _day
        number_lesson = (
            self.get_lesson_number_now_without_type(_debug=False) if _number_lesson is None else _number_lesson
        )
        type = self.get_week_type(_debug=False) if _type is None else _type
    ...
    def get_day_isoweekly(self, year: int = None, mounth: int = None, day: int = None) -> int:
        """
    ...
    def get_week_type(
        ...
        when_mounth, when_day, when_year = (
            (mounth if mounth else self.database.get_data_week_type(_debug=False)["month"]),
        ...
        for i in range(mounth - when_mounth + 1):
            for j in range(
                1,
                calendar.monthrange(
                    year if when_mounth + i > 12 else year + 1,
                    i + when_mounth if when_mounth + 1 > 12 else 1,
                )[1]
                + 1,
            ):
    ...
    def get_number_lesson_now(self) -> np.int8:
        ...
        for number_lesson in times:
            if (
                self.get_time_float(_time=times[number_lesson]["start_recess"], _debug=False)
                <= time_now
                <= self.get_time_float(_time=times[number_lesson]["start_lessons"], _debug=False)
            ):
            ...
            if (
                self.get_time_float(_time=times[number_lesson]["start_lessons"], _debug=False)
                <= time_now
                <= self.get_time_float(_time=times[number_lesson]["end_lessons"], _debug=False)
            ):
                return np.int8(int(number_lesson))
    ...
    def get_time_now(self, _debug: bool = True) -> datetime.datetime:
        ...
        if _debug:
            print_debug(
                "System",
                f"Get [main]time now[/main]: [main]{time_now}[/main] in [main]{time_zone}[/main].",
            )
        ...
        if not mass:
            ...
            if type(mass) is list or type(mass) is tuple:
                ...
                for number_lesson in mass:
                    ...
                    self.cancellation.append([number_lesson, self.get_day_isoweekday_now(_debug=False)])
    ...
    def get_cancellation(self, _debug: bool = True) -> bool:
        ...
        for i in range(len(self.cancellation)):
            if self.cancellation[i][0] == self.get_lesson_number_now_without_type(_debug=False) and self.cancellation[
                i
            ][1] == self.get_day_isoweekday_now(_debug=False):
                return True
