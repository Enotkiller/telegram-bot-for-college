...
class DataBase:
    ...
    def converter_data_in_redis(self, _debug: bool = True) -> None:
            ...
            for number_lesson, lessons in self.schedule[day].items():
                ...
                if lessons_data:
                    ...
                    if _debug:
                        print_debug(
                            "DataBase",
                            f"redis add schedule: [main]{day}[/main], number_lesson: [main]{number_lesson}[/main], lessons: [main]{lessons_data}[/main].",
                        )
        ...
        if _debug:
            print_debug(
                "DataBase",
                f"redis add [main]pings_username[/main]: [main]{pings_username_data}[/main].",
            )
    ...
    def get_id_lesson_for_date(
        ...
        if _debug:
            print_debug(
                "DataBase",
                f"Get id lesson for date: day: [main]{day}[/main], number lesson: [main]{number_lesson}[/main], lessons: [main]{lessons}[/main].",
            )
    ...
    def get_lesson_for_date(
        ...
        if _debug:
            print_debug(
                "DataBase",
                f"Get lesson for date: day: [main]{day}[/main], number lesson: [main]{number_lesson}[/main], lessons: [main]{lessons}[/main].",
            )
    ...
    def get_number_of_weeks_types(self, _debug: bool = True) -> np.uint8:
        ...
        if _debug:
            print_debug(
                "DataBase",
                "Get [main]number of weeks types[/main], number of weeks types: [main]"
                + str(number_of_weeks_types)
                + "[/main].",
            )
    ...
    def get_send_spam(self, number_lesson: int = None, _debug: bool = True) -> str:
        ...
        if _debug:
            print_debug(
                "DataBase",
                "Get [main]send spam[/main]: [main]"
                + send_spam
                + "[/main], number lesson: [main]"
                + str(number_lesson)
                + "[/main].",
            )
